...
def testmail(
    ...
    # One per label, plus the two decision paths that are not about filing.
    # In Polish on purpose: this mailbox receives Polish mail, and a label
    # picked from an English test says nothing about how it will behave on the
    # real thing.
    tail = "\n\n-- Wiadomosc testowa z email-workflow. --"
    samples = [
        (
            "[TEST] Twoja przesylka 6000123456 jest w drodze",
            "Twoja paczka zostala nadana i jest w drodze do paczkomatu "
            "KRA01M. Numer przesylki: 6000123456." + tail,
            "paczka -> Zakupy",
        ),
        (
            "[TEST] -30% na wszystko, tylko dzis!",
            "Wyprzedaz jesienna! -30% na cala kolekcje z kodem TEST30. "
            "Oferta wazna do polnocy." + tail,
            "rabat -> Rabaty, i znika z inboxa",
        ),
        (
            "[TEST] Oferta pracy - Przedstawiciel Handlowy",
            "Znalezlismy oferte, ktora moze Cie zainteresowac: "
            "Przedstawiciel Handlowy, malopolskie. Ostatnia szansa na "
            "wyslanie CV." + tail,
            "praca -> Praca, zostaje w inboxie",
        ),
        (
            "[TEST] Faktura za wrzesien 2026",
            "W zalaczniku faktura nr TEST-0001 na kwote 89,00 zl. "
            "Termin platnosci: 30 dni." + tail,
            "faktura -> Finanse, zostaje i dostaje Priorytet",
        ),
        (
            "[TEST] Nowe logowanie na Twoje konto",
            "Wykryto logowanie z nowego urzadzenia (Windows, Krakow). "
            "Jesli to nie Ty, natychmiast zmien haslo." + tail,
            "bezpieczenstwo -> Bezpieczenstwo, zostaje i dostaje Priorytet",
        ),
        (
            "[TEST] Newsletter - co nowego w tym tygodniu",
            "W tym tygodniu: piec porad, dwa linki i zdjecie psa. "
            "Mozesz zrezygnowac w kazdej chwili." + tail,
            "newsletter -> Newslettery, i znika z inboxa",
        ),
        (
            "[TEST] Krotkie pytanie o godziny pracy",
            "Czesc,\n\nW jakich godzinach pracujesz w tym tygodniu?"
            "\n\nDzieki,\nAnna" + tail,
            "da sie odpowiedziec z Known Facts -> moze wyslac odpowiedz",
        ),
        (
            "[TEST] Potwierdzisz budzet na przyszly kwartal?",
            "Czesc,\n\nPotwierdz prosze dokladna kwote budzetu na Q1 "
            "i termin startu. Potrzebuje tego dzis.\n\nMarek" + tail,
            "brakuje faktu -> NIE moze odpowiedziec sam",
        ),
    ]
